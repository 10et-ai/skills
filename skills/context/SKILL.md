...
Query the TENET context system — journal entries, memory, knowledge docs, and decisions.
...
- **`tenet_context`** — Search all project context (journal, knowledge, memory)
- **`tenet_memory_search`** — Semantic search across session memories
- **`tenet_synopsis`** — Aggregated work summary across sessions

## Context Sources

1. **Journal** (`.tenet/journal/<branch>.jsonl`) — Session-level work log
...
1. Ensures Context Hub is running (`tenet context-hub ensure`)
2. Before each agent turn, fetches recent context (`GET /api/context?limit=10`)
3. Injects into system prompt as a `## TENET Project Context` section
...
```typescript
// In extension code:
const result = await ctx.tools.tenet_context({ query: "what did we decide about pricing?", limit: 5 })
```
...
Run `tenet context-hub status` to check health.
