...
1. Loads the `@tenet-ai/pi` extension (context injection, journal, MAP bridge)
...
```bash
tenet pi agents run --team teams/gtm-team.yaml
```
...
5. Max 5 iterations (configurable in `.tenet/config.json` → `pi.max_peter_iterations`)
...
For backward compatibility, ralph-tui workflows still work — use the separate `tenet peter` command.
