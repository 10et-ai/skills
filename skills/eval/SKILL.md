...
2. Entry written to `.tenet/eval.jsonl` via `eval-store.ts`
...
- Training tuples written to `~/.tenet/training-buffer.jsonl`
...
```bash
cat .tenet/eval.jsonl | tail -20    # Recent evals
tenet eval report                   # Formatted report
```
